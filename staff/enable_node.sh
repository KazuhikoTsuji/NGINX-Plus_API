...
read -p "Which node do you enable? Please enter ID of node.: " ID
...
read -p "You are about to enable following node from existing environment.
        [$HOST:$PORT weight:$WEIGHT]
Do you want to proceed? [y/n] " ANSWER
if [ "$ANSWER" = "y" ];
then
  curl -X PATCH -d '{ "down": false }' -s "http://$TARGET_IP:$TARGET_PORT/api/3/http/upstreams/$UPSTREAM/servers/$ID" > /dev/null 2>&1
  exit 0
else
  exit 1
fi
